...
def cargarCSV(todos):
    ...
    path = os.path.join(module_dir, "on", "*.csv")
...
def iniciarTabla():
    ...
    module_dir = os.path.dirname(__file__)
    module_dir = os.path.join(module_dir, r"..",r"funciones",r"corporate_bonds.csv")
    ...
    for i in todos:
        ...
        try:
            ...
            if np.isnan(loc) == False:
                dicAPI[i+"o"] = float(loc)*-1
                #print("LAST---------------------- in", i)
            elif np.isnan(locPrev) == False:
                dicAPI[i+"o"] = float(locPrev)*-1
                #print("PREVIOUS---------------------- in", i)
            else:
                todos.remove(i)
        except Exception as e:
            print("Error descargando ", i, " en On desde la API: ", e)
            todos.remove(i)
