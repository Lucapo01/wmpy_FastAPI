...
def cargarCSV(todos):
    ...
    path = os.path.join(module_dir, "dolar", "*.csv")
...
def graficar(mat):
    ...
    diquiFinal = {}
    diquiFinal["titulo"] = "ON"
    diquiFinal["nombre"] = "Curva On"
    listaPuntos = []
...
def separarCurvas(graficoInicial):
    graficoSeparado = []
    puntosA=[]
    puntosG=[]
    dicA = {}
    ...
    dicA["titulo"] = "DOLAR"
    dicA["nombre"] = "Curva Ley Nacional"
    dicG["titulo"] = "DOLAR"
    dicG["nombre"] = "Curva Ley Extranjera"

    for i in graficoInicial[0]["puntos"]:
        if i["ticker"].startswith("a"):
            puntosA.append(i)
        elif i["ticker"].startswith("g"):
            puntosG.append(i)
        else:
            print("Error en la separacion de ", i["ticker"])

    dicA["puntos"] = puntosA
    dicG["puntos"] = puntosG

    graficoSeparado.append(dicA)
...
def iniciarTabla():
    ...
    graficoSeparado = separarCurvas(graficoInicial)

    print("Dolar Actualizado")
